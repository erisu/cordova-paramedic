...
class ParamedicApp {
    constructor (config, storedCWD, runner) {
        ...
        this.platformOverride = this.config.getPlatformOverride();
     ...
     */
    installPlatform () {
        const platformToInstall = this?.platformOverride ?? this.platformId;
        return spawnAsync(
            this.config.getCli(),
            ['platform', 'add', platformToInstall, ...utilities.PARAMEDIC_COMMON_ARGS],
