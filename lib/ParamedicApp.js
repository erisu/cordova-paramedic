...
const { utilities, execPromise, logger, exec } = require('./utils');
...
class ParamedicApp {
    ...
    installPlugins () {
        ...
        if (this.platformId === utilities.WINDOWS) {
            additionalPlugins.push(path.join(__dirname, '../debug-mode-plugin'));
        }

        if (this.platformId === utilities.IOS) {
    ...
    installPlatform () {
        ...
        return execPromise(this.config.getCli() + ' platform add ' + platform + utilities.PARAMEDIC_COMMON_CLI_ARGS + utilities.PARAMEDIC_PLATFORM_ADD_ARGS)
            .then(() => {
                ...
                if (this.platformId === utilities.ANDROID && this.config.isCI()) {
                ...
                } else if (this.platformId === utilities.BROWSER && this.config.shouldUseSauce()) {
                    ...
                    var cordovaRunFile = path.join(this.tempFolder.name, 'platforms/browser/cordova/run');
    ...
    checkPlatformRequirements () {
        if (this.platformId === utilities.BROWSER) return Promise.resolve();
