...
class AppiumRunner {
    ...
    #prepareOptions (options) {
        ...
        this.options.targetDevice = this.options.appiumDeviceName;
    ...
    prepareApp () {
        return new Promise((resolve, reject) => {
            ...
            fs.stat(fullAppPath, async (error, stats) => {
                ...
                if (
                    buildResult.code !== 0
                    || buildResult.stdout.includes('BUILD FAILED')
                    || buildResult.stderr.includes('BUILD FAILED')
                ) {
                    reject(new Error('Failed to build app with following error: ' + buildResult.stderr));
                } else {
