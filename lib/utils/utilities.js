...
function getSimulatorCollection () {
    ...
    const instrCommand = 'xcrun xctrace list devices | grep ^iPhone';
    logger.info('running:');
...
function getSimulatorData (findSimResult) {
    ...
    // This matches <device> Simulator (<version>) (<simulator-id>)
    const simIdRegex = /^([a-zA-Z\d ]+) Simulator \(([\d.]+)\) \(([a-zA-Z\d-]*)\).*$/;
