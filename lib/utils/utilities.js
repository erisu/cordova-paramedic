...
function getSimulatorCollection (deviceListBin) {
    if (simulatorCollection) return simulatorCollection;

    /**
     * We will grab the list of devices and filter only for iPhone items.
     * xcrun xctrace prints out to stderr so we will redirect the messages to stdout with "2>&1".
     * Then we will continue to grep as usual.
     *
     * Keep instruments as fallback until 12.x is removed from testing plan
     */
...
function filterForSimulatorIds (simulatorData, simulatorCollection, simIdRegex) {
    return simulatorCollection
        .reduce((result, line) => {
...
function getSimulatorData (findSimResult, deviceListBin) {
    ...
    // Default is "xcrun", use "instruments" when defined manually in configs.
    let simIdRegex = deviceListBin === 'instruments'
        ? /^([a-zA-Z\d ]+) \(([\d.]+)\) \[([a-zA-Z\d-]*)\].*$/
        : /^([a-zA-Z\d ]+) Simulator \(([\d.]+)\) \(([a-zA-Z\d-]*)\).*$/;

    // Fetch the environment's installed simulator collection data
    const simulators = getSimulatorCollection(deviceListBin);

    // Try to find the simulator ids from the simulator collection against the data & expression
    let simulatorIds = filterForSimulatorIds(simulatorData, simulators, simIdRegex);

    // If not instruments and no simulator IDs, try the "xcrun" v1 pattern that was used in Xcode 12.x.
    if (deviceListBin !== 'instruments' && simulatorIds.length === 0) {
        simIdRegex = /^([a-zA-Z\d ]+) \(([\d.]+)\) \(([a-zA-Z\d-]*)\).*$/;
        simulatorIds = filterForSimulatorIds(simulatorData, simulators, simIdRegex);
    }
