...
class ParamedicRunner {
    ...
    async waitForTests (cmdArgs) {
        ...
        // Added abort controller to terminate "cordova run" after jasmine triggeres the done event or disconnect.
        ...
        const testResults = new Promise((resolve, reject) => {
            ...
            this.server.on('disconnect', () => {
                // Terminate the runnings of cordova.
                controller.abort();
        ...
        try {
            await spawnAsync(
                this.config.getCli(),
                cmdArgs,
                { cwd: this.tempFolder.name, signal: controller.signal }
            );
        } catch (e) {
            logger.info('[paramedic] "cordova run" process has been completed.');
        }
    ...
    async getRunLocalTestCommandArgs () {
        ...
        if (this.isBrowser || this.config.getAction() === 'build') {
            if (this.isBrowser && this.config.getAction() === 'run' && process.env.CI) {
                args.push('--target=chrome');
                // args.push('--');
