...
class ParamedicRunner {
    constructor (config) {
        ...
        this.isMacOS = this.config.getTarget() === utilities.MACOS;
        this.isIos = !this.isMacOS && this.config.getPlatformId() === utilities.IOS;
    ...
    runAppiumTests (useSauce) {
        ...
        const platform = this.config.getTarget === utilities.MACOS
            ? utilities.MACOS
    ...
    getCommandForStartingTests () {
        ...
        return new ParamedicTargetChooser(this.tempFolder.name, this.config).chooseTarget(
            ...
            return cmd
                ...
                .concat(this.isIos || this.isMacOS ? ['--emulator'] : [])
                .join(' ');
        });
    }
