...
class ParamedicRunner {
    ...
    getCommandForStartingTests () {
        let cmd = [
            ...
            .concat(this.isBrowser ? [this.config.getArgs()] : []);

        console.log('[initial command]', cmd.join());

        if (this.isBrowser) {
            console.log('[browser command]', cmd.join());

            return cmd.join(' ');
        } else if (this.config.getAction() === 'build' || (this.isWindows && this.config.getArgs().indexOf('appx=8.1-phone') < 0)) {
            console.log('[build and or windows command]', cmd.join());
            ...
            return Q().then(
                () => (new ParamedicTargetChooser(this.tempFolder.name, this.config))
                    .chooseTarget(
                        true, // useEmulator
                        this.config.getTarget() // preferredTarget
                    )
            ).then(targetObj => {
                console.log('[everything else target]', targetObj);
                this.targetObj = targetObj;

                console.log('[isIos?]', this.isIos);
                console.log('[final commands]', cmd
