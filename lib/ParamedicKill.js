...
class ParamedicKill {
    ...
    kill () {
        ...
        if (platformTasks.length < 1) {
            logger.warn('no known tasks to kill');
            return;
        }
    ...
    tasksOnPlatform (platformName) {
        ...
        case utilities.BROWSER:
            tasks = utilities.isWindows() ? ['chrome.exe'] : ['chrome'];
        }
    ...
    killTasks (taskNames) {
        ...
        if (killTasksResult.code !== 0) {
            logger.warn('WARNING: kill command returned ' + killTasksResult.code);
        }
    }
    ...
    killAdbServer () {
        ...
        logger.info(`Running the following command: ${killServerCommand}`);
